...
class Ls(arguments: List<String>, prev: Executable?) :
    ...
    override fun processEmptyInput(): String {
        var result = ""
        File(GlobalEnvironment.userDir).listFiles().forEach { result = result + it.name + " " + (if (it.isDirectory) "directory" else "file")+ "\n" }
        return result
    }
