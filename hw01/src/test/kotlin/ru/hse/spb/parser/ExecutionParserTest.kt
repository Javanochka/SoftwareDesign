...
class ExecutionParserTest {
    ...
    @Test
    fun cat() {
        testParsing<Cat>(listOf("cat", "example.txt"), Cat(listOf(Paths.get(GlobalEnvironment.userDir, "example.txt")), null))
    }

    @Test
    fun catPipeWc() {
        testParsing<Wc>(listOf("cat", "example.txt", "|", "wc"),
            Wc(listOf(), Cat(listOf(Paths.get(GlobalEnvironment.userDir, "example.txt")), null)))
    }
